	...
	public static ArrayList<Tournament> getTournaments(SQLUtilities sql, Settings settings, String api) {
		...
		for (JSONObject j : allJson) {
			...
			if (!j.isNull("start_at")) {
				...
				//this is the date the "start tournament" button was pressed by an admin
				//if the date to start wasn't specified, this works
			...
			//this can be done before sorting, because it is going to be inserted with a primary key of id, given by challonge
			//thus, the sorting doesn't matter, as the id is not being autogenerated
			sql.insertTournament(toAdd);
		}
		
		//this is done so that the matches are processed in the proper order, to ensure valid and fair elo calculations
		toReturn.sort(new SortByDate());
...
/*
 * Comparator to sort tournaments by date started
 * 
 * Not consistent with equals
 */
class SortByDate implements Comparator<Tournament> {
	public int compare(Tournament a, Tournament b) {
		...
		try {
			aDate = (Date) df.parse(a.dateStarted);
			bDate = (Date) df.parse(b.dateStarted);
